...
log = logging.getLogger("Form3Parser")
...
# Map XML element paths to canonical section label
XML_SECTION_MAP: Dict[str, str] = {
    "root": "Cover Page",  # misc root fields
    "issuer": "Issuer Information",
    "reportingOwner": "Reporting Owner Information",
    "nonDerivativeTable": "Table I – Non-Derivative Securities Beneficially Owned",
    "derivativeTable": "Table II – Derivative Securities Beneficially Owned",
...
def markdown_table(headers: List[str], rows: List[List[str]]) -> str:
    md = "|" + "|".join(headers) + "|\n" if headers else "|" + "|".join(["" for _ in rows[0]]) + "|\n"
    md += "|" + "|".join(["---" for _ in (headers or rows[0])]) + "|\n"
    for row in rows:
        md += "|" + "|".join(row) + "|\n"
    return md.strip()


def parse_form3_xml(fp: pathlib.Path, link_map: Dict[str, str]) -> Tuple[Dict[str, Any], set[str]]:
    """Parse a single Form 3 XML file to structured dict & missing-URL set"""
    ...
    ticker, _, year = rel_parts[:3]  # <TICKER>/3/<YEAR>/file.xml

    tree = ET.parse(fp)
    root = tree.getroot()

    # Build base structure for all canonical sections
    sections: Dict[str, Dict[str, Any]] = {
        s: {
            "html_blocks": [],  # generic name kept for uniformity
            "subsections": {},
            "tables": [],
            "chunks": [],
            "missing": True,
        } for s in FORM3_CANON_SECTIONS
    }
    ...
    add_block("Cover Page", f"Document Type: {doc_type}\nPeriod of Report: {period}\nSchema Version: {schema}")
    ...
    if non_derivative is not None:
        ...
        add_block("Table I – Non-Derivative Securities Beneficially Owned", md)
        sections["Table I – Non-Derivative Securities Beneficially Owned"]["tables"].append({"headers": headers, "data": rows})
    ...
    if derivative is not None:
        ...
        md = markdown_table(headers, rows)
        add_block("Table II – Derivative Securities Beneficially Owned", md)
        sections["Table II – Derivative Securities Beneficially Owned"]["tables"].append({"headers": headers, "data": rows})
...
def parse3():
    ...
    with tqdm(total=len(worklist), desc="Processing Form 3s", unit="file", ncols=90) as bar:
        for ticker, year, fp, link_map in worklist:
            out_dir = OUT_DIR / ticker / FILING_TYPE_DIRNAME / year
            out_dir.mkdir(parents=True, exist_ok=True)
            try:
                parsed, misses = parse_form3_xml(fp, link_map)
